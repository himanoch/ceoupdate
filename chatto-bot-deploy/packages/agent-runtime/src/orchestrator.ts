...
export class AgentOrchestrator {
  ...
  public async processInboundMessage(params: {
    ...
    if (this.llmGateway) {
      const llmDecision = await this.llmGateway.decide({
        conversationId: params.conversationId,
        contactId: params.contactId,
        message: params.messageContent,
        persona,
        systemPrompt: activePrompt?.systemPrompt || 'ผู้ช่วยอัจฉริยะที่ช่วยตอบคำถามและจัดการข้อมูลลูกค้า',
      });

      if (llmDecision.action === 'tool_call' && llmDecision.toolCalls?.length) {
        const toolResults = [] as any[];
        for (const call of llmDecision.toolCalls) {
          const result = await this.llmGateway.executeToolPlan(call);
          toolResults.push(result);
          toolCallsExecuted.push(call.toolName);

          if (call.toolName === 'search_properties') {
            const plots = Array.isArray(result) ? result : [];
            const plotListText = plots.length > 0
              ? plots.slice(0, 3).map((p: any) => `• แปลง ${p.plotNumber} (${p.projectName} - ${p.location})\n  - ขนาด: ${p.areaSqWah} ตร.ว. (${p.areaSqm} ตร.ม.)\n  - เอกสารสิทธิ์: ${p.titleDeed}\n  - ราคา: ${p.priceThb.toLocaleString('th-TH')} บาท (อัปเดต ณ ${p.effectiveDate})\n  - จุดเด่น: ${p.features.join(', ')}`).join('\n\n')
              : 'ไม่พบแปลงที่ตรงกับเงื่อนไขในขณะนี้';

            const replyText = `สวัสดีค่ะ ${persona}ขอแนะนำแปลงที่ดินวิวเขาสวย โฉนดครุฑแดงแท้ของ The Hill Land ดังนี้ค่ะ:\n\n${plotListText}\n\n✨ โปรโมชั่นพิเศษ: ผ่อนตรงกับโครงการ 0% นานสูงสุด 36 เดือน ไม่เช็คเครดิตบูโร\n\nหากสนใจแปลงไหนเป็นพิเศษ หรือต้องการนัดชมแปลงจริง สามารถแจ้งเบอร์โทรหรือวันที่สะดวกเพื่อประสานงานฝ่ายขายได้เลยนะคะ 😊`;
            const reply = this.crm.addMessage({
              conversationId: params.conversationId,
              senderType: 'agent_ai',
              content: replyText,
            ...
            return { status: 'replied', replyMessage: reply, toolCallsExecuted, citations };
          }

          if (call.toolName === 'create_lead') {
            const lead = result;
            const replyText = `บันทึกข้อมูลความสนใจของคุณเรียบร้อยแล้วค่ะ (รหัส Lead: #${lead.id})\n\nเจ้าหน้าที่ฝ่ายขาย The Hill Land จะติดต่อกลับทางเบอร์โทร ${lead.phone} เพื่อส่งผังแปลงโฉนดและนัดหมายวันเข้าชมโครงการจริงนะคะ\n\nหากต้องการข้อมูลเพิ่มเติม สามารถสอบถาม${persona}ได้ตลอดเวลาค่ะ`;
            const reply = this.crm.addMessage({
              conversationId: params.conversationId,
              senderType: 'agent_ai',
              content: replyText,
          ...
          if (call.toolName === 'handoff_to_human') {
            const replyText = 'รับเรื่องส่งต่อให้เจ้าหน้าที่ฝ่ายบริการ The Hill Land เรียบร้อยแล้วค่ะ เจ้าหน้าที่จะเข้ามาดูแลและติดต่อกลับทางช่องทางนี้โดยเร็วที่สุดนะคะ (ระบบ AI หยุดการตอบอัตโนมัติชั่วคราวเพื่อให้ทีมงานดูแลต่อค่ะ)';
            const reply = this.crm.addMessage({
              conversationId: params.conversationId,
              senderType: 'agent_ai',
              content: replyText,
            });
            return { status: 'handed_off', replyMessage: reply, toolCallsExecuted, botPaused: true };
          }
        }
      }

      if (llmDecision.action === 'reply' && llmDecision.replyText) {
        const reply = this.crm.addMessage({
          conversationId: params.conversationId,
          senderType: 'agent_ai',
          content: llmDecision.replyText,
        });
        return { status: 'replied', replyMessage: reply, citations: [] };
      }
    }

    // 3. Search Knowledge Base
    const kbMatches = this.kb.search(params.messageContent);
    citations = kbMatches.map((k) => ({
      sourceId: k.sourceId,
      title: k.title,
